...
(let ((h (make-hash-table :test 'equal)))
  ...
  (puthash "background" " <bg-image> || <position> [ / <bg-size> ]? || <repeat-style> || <attachment> || <box>{1,2} " h)
  (puthash "background-attachment" "<scroll | fixed | local> [ , <scroll | fixed | local> ]* " h)
  (puthash "background-color" "color | transparent | inherit" h)
  (puthash "background-clip" "<border-box | padding-box | content-box> [ , <box> ]* " h)
  (puthash "background-image" "none | <bg-image> [ , <bg-image> ]* " h)
  (puthash "background-origin" "<border-box | padding-box | content-box> [ , <box> ]* " h)
  (puthash "background-size" "[ <length> | <percentage> | auto ]{1,2} | cover | contain [ , <bg-size> ]* " h)
  (puthash "background-position" "left | center | right | top | bottom | <percentage> | <length> [, <position> ]* " h)
  (puthash "background-repeat" "repeat-x | repeat-y | [repeat | space | round | no-repeat]{1,2}" h)
  ...
  (puthash "border-style" "<none | hidden | dotted | dashed | solid | double | groove | ridge | inset | outset>{1,4} | inherit" h)
  ...
  (puthash "border-image" "<‘border-image-source’> || <‘border-image-slice’> [ / <‘border-image-width’> | / <‘border-image-width’>? / <‘border-image-outset’> ]? || <‘border-image-repeat’>" h)
  (puthash "border-image-source" "none | <image>" h)
  (puthash "border-image-slice" "[<number> | <percentage>]{1,4} && fill? " h)
  (puthash "border-image-width" "[ <length> | <percentage> | <number> | auto ]{1,4}" h)
  (puthash "border-image-outset" "[ <length> | <number> ]{1,4}" h)
  (puthash "border-image-repeat" "[ stretch | repeat | round | space ]{1,2}" h)
  (puthash "border-left-style" "style | inherit" h)
  (puthash "border-top-width" "width | inherit" h)
  (puthash "border-top-left-radius" "[ <length> | <percentage> ]{1,2}" h)
  (puthash "border-top-right-radius" "[ <length> | <percentage> ]{1,2}" h)
  (puthash "border-bottom-right-radius" "[ <length> | <percentage> ]{1,2}" h)
  (puthash "border-bottom-left-radius" "[ <length> | <percentage> ]{1,2}" h)
  (puthash "border-bottom-width" "width | inherit" h)
  (puthash "border-right-width" "width | inherit" h)
  (puthash "border-radius" "[ <length> | <percentage> ]{1,4} [ / [ <length> | <percentage> ]{1,4} ]? " h)
  (puthash "border-left-width" "width | inherit" h)
  (puthash "border-width" "<<length> | thin | medium | thick> {1,4} | inherit" h)
  (puthash "bottom" "length | percentage | auto | inherit" h)
  (puthash "box-decoration-break" "slice | clone" h)
  (puthash "box-shadow" "<length>{2,4} && <color>" h)
  (puthash "break-after" "auto | always | avoid | left | right | page | column | avoid-page | avoid-column" h)
  (puthash "break-before" "auto | always | avoid | left | right | page | column | avoid-page | avoid-column" h)
  (puthash "break-inside" "auto | avoid | avoid-page | avoid-column" h)
  ...
  (puthash "columns" "<‘column-width’> || <‘column-count’>" h)
  (puthash "column-count" "<integer> | auto" h)
  (puthash "column-fill" "auto | balance" h)
  (puthash "column-gap" "<length> | normal" h)
  (puthash "column-rule" "<‘column-rule-width’> || <‘column-rule-style’> ||  <‘column-rule-color’>" h)
  (puthash "column-rule-color" "<color>" h)
  (puthash "column-rule-style" "none | hidden | dotted | dashed | solid | double | groove | ridge | inset | outset" h)
  (puthash "column-rule-width" "<border-width>" h)
  (puthash "column-span" "none | all" h)
  (puthash "column-width" "<length> | auto" h)
  ...
  (puthash "cue-after" "<uri> <decibel>? | none" h)
  (puthash "cue-before" "<uri> <decibel>? | none" h)
  (puthash "cue" "<cue-before> <cue-after>" h)
  ...
  (puthash "image-orientation" "<angle>" h)
  (puthash "image-resolution" "[from-image || <resolution>] && snap?" h)
  ...
  (puthash "marquee-direction" "forward | reverse" h)
  (puthash "marquee-play-count" "<non-negative-integer> | infinite" h)
  (puthash "marquee-speed" "slow | normal | fast" h)
  (puthash "marquee-style" "scroll | slide | alternate" h)
  (puthash "overflow-style" "auto | marquee-line | marquee-block" h)
  ...
  (puthash "object-fit" "fill | contain | cover | none | scale-down" h)
  (puthash "object-position" "<position>" h)
  ...
  (puthash "pause-after" "<time> | none | x-weak | weak | medium | strong | x-strong" h)
  (puthash "pause-before" "<time> | none | x-weak | weak | medium | strong | x-strong" h)
  (puthash "pause" "<‘pause-before’> <‘pause-after’>?" h)
  ...
  (puthash "rest" "<‘rest-before’> <‘rest-after’>?" h)
  (puthash "rest-after" "<time> | none | x-weak | weak | medium | strong | x-strong" h)
  (puthash "rest-before" "<time> | none | x-weak | weak | medium | strong | x-strong" h)
  ...
  (puthash "speak-as" "normal | spell-out || digits || [ literal-punctuation | no-punctuation ]" h)
  ...
  (puthash "voice-balance" "<number> | left | center | right | leftwards | rightwards" h)
  (puthash "voice-family" "[[<specific-voice> | <generic-voice> ],]* [<specific-voice> | <generic-voice> ] | inherit" h)
  (puthash "voice-duration" "auto | <time>" h)
  (puthash "voice-family" "[[<name> | <generic-voice>],]* [<name> | <generic-voice>] | preserve" h)
  (puthash "voice-pitch" "<frequency> && absolute | [[x-low | low | medium | high | x-high] || [<frequency> | <semitones> | <percentage>]]" h)
  (puthash "voice-range" "<frequency> && absolute | [[x-low | low | medium | high | x-high] || [<frequency> | <semitones> | <percentage>]]" h)
  (puthash "voice-rate" "[normal | x-slow | slow | medium | fast | x-fast] || <percentage>" h)
  (puthash "voice-stress" "normal | strong | moderate | none | reduced" h)
  (puthash "voice-volume" "silent | [[x-soft | soft | medium | loud | x-loud] || <decibel>]" h)
